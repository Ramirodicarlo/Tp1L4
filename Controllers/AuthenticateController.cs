...
namespace TODOLIST.Controllers
{
    ...
    public class AuthenticateController : ControllerBase
    {
        ...
        {
            if (credentialsDto != null && credentialsDto.Email != null )
            {
                // Valido usuario
                BaseResponse validateUserResult = _userService.ValidateUser(credentialsDto.Email);
                if (validateUserResult.Message == "wrong email")
                {
                    return BadRequest(validateUserResult.Message);
                }
                else if (validateUserResult.Result)
                {
                    // Obtener usuario
